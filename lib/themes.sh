...
setup_wallpapers() {
    ...
    local wallpaper_source="$repo_dir/assets/main-wallpaper.jpg"
    local fallback_wallpaper_source="$repo_dir/assets/wallpapers/main-wallpaper.png"
    local legacy_wallpaper_source="$repo_dir/assets/wallpapers/wallpaper.png"

    if [ ! -f "$wallpaper_source" ] && [ -f "$fallback_wallpaper_source" ]; then
        wallpaper_source="$fallback_wallpaper_source"
    fi
    ...
    if [ "$install_hyprland" = "true" ]; then
        ...
        ln -sf "$wallpaper_source" "$hypr_wallpaper_dir/main-wallpaper.jpg"
        ln -sf "$wallpaper_source" "$hypr_wallpaper_dir/main-wallpaper.png"
        ln -sf "$wallpaper_source" "$hypr_wallpaper_dir/wallpaper.png"

        if [ "$EUID" -eq 0 ]; then
            chown -h "$user:$user" "$hypr_wallpaper_dir/main-wallpaper.jpg"
    ...
    if [ "$install_niri" = "true" ]; then
        ...
        ln -sf "$wallpaper_source" "$niri_wallpaper_dir/main-wallpaper.jpg"
        ln -sf "$wallpaper_source" "$niri_wallpaper_dir/main-wallpaper.png"
        ln -sf "$wallpaper_source" "$niri_wallpaper_dir/wallpaper.png"

        if [ "$EUID" -eq 0 ]; then
            chown -h "$user:$user" "$niri_wallpaper_dir/main-wallpaper.jpg"
